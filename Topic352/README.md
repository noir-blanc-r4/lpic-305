...
| OCIが提供するコンテナイメージの標準規格は？（略語で） | image-spec | <span style="color: red; ">×</span> |
| DockerやKubernetesとの互換性があり、コンテナイメージを構築（ビルド）する際に利用されるコンテナ管理ツールは？ | Buildah | <span style="color: red; ">×</span> |
| コンテナイメージの検査や署名、コピーや転送などを行うコンテナ管理ツールは？ | Skopeo | <span style="color: red; ">×</span> |
...
| それぞれの説明にあうcgroupのコントローラ（サブシステム）を答えよ <br> ・CPUアクセスのスケジューリング <br>・メモリリソースのレポートの作成やメモリ使用量の上限設定 | `cpu` <br> `memory` | ○ <br> ○ |
| それぞれの説明にあうcgroupのコントローラ（サブシステム）を答えよ <br>・ブロックデバイスに対するI/Oへのアクセス制御および監視 <br> ・CPUおよびメモリノードの割り当て | `blkio` <br> `cpuset` | <span style="color: red; ">×</span> <br> <span style="color: red; ">×</span> |
| それぞれの説明にあうcgroupのコントローラ（サブシステム）を答えよ <br>・デバイスへのアクセス制御 <br>・プロセスの一時停止または再開 <br>・ネットワークトラフィックの優先度を設定 <br>・cgroupから発信されるパケットをタグ付けして制御 | `devices` <br> `freezer` <br> `net_prio` <br> `net_cls` | <span style="color: red; ">×</span> <br> <span style="color: red; ">×</span> <br> ○ <br> <span style="color: red; ">×</span> |
...
| プロセスIDが22222で動作しているコンテナが属している名前空間を確認できるコマンドは？コマンド名が短い方から順に答えよ | **①** ```ls -l /proc/22222/ns``` <br> **②** ```lsns -p 22222``` | <span style="color: red; ">×</span> <br> <span style="color: red; ">×</span> |
